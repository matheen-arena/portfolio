...
- **Hero (cockpit flight)**: a first-person spaceship cockpit flying over a 3D night city (three.js, bundled in `js/vendor/`). Scrolling flies the camera forward through three stages: intro, the Logz.io → Grafana OSS migration, and the GitLab → GitHub migration. After that the page scrolls normally. The dashboard has a radar, a live log and a "scroll down" console that jumps to the next stage. Without WebGL, a static sky shows behind the cockpit.
